...
private:
  // Parameters
  const static int NUM = 50;       // NUmber of search for target
...

  {
    ...
    if (res.empty())
    {
      if (wait_reverse_cnt < wait_threshold)
      {
        ...
        ros::Publisher pub_stop_cmd = nh_.advertise<std_msgs::Bool>("arrive", 1);
      ...
      if (res.empty())
      {
        ...
        planned_path.header.frame_id = "camera_link";
        ros::Publisher pub_stop_cmd = nh_.advertise<std_msgs::Bool>("arrive", 1);
        std_msgs::Bool stop_cmd;
        stop_cmd.data = true;
        pub_stop_cmd.publish(stop_cmd);
...
public:
  WallFollower(ros::NodeHandle nh, ros::NodeHandle pnh) : nh_(nh), pnh_(pnh), count(0), count_reverse(0), stuck_counter(0), wait_reverse_cnt(0)
  {
    ...
    {
      for (int i = 0; i < NUM; ++i)
        angle[i] = 28 + i * -60. / (NUM - 1); // Degree [60, -75]
      for (int i = 0; i < NUM; ++i)
        reverse[i] = -90 - i * 120. / (NUM - 1); // Degree [-90, -210]
    }
    else
