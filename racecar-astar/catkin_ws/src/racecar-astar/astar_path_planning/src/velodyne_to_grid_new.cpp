...
public:
  ...
  void map2occupancygrid(float&, float&,float&, float&);
...
Obstacle_Detection::Obstacle_Detection(ros::NodeHandle &n, ros::NodeHandle &pn):
  ...
  cout <<occupancygrid<< endl;
  // ROS_INFO(occupancygrid);
  // printf(std::to_string(occupancygrid.info.width));
  // printf(std::to_string(-map_size*map_resolution/2));
  ...
  // Subscriber
  sub_cloud = pnh.subscribe("/husky1/velodyne1/velodyne_points", 1, &Obstacle_Detection::cbCloud, this);
  // sub_cloud = pnh.subscribe("/camera/depth/color/points", 1, &Obstacle_Detection::cbCloud, this);
}
...
void Obstacle_Detection::map2occupancygrid(float& x, float& y, float& a, float& b){
  b = int(occupancygrid.info.origin.position.x);
  a = int(occupancygrid.info.origin.position.y);
  cout <<b<< endl;
  cout <<a<< endl;
  // printf('%f\n',b);
  // printf('%f\n',a);
