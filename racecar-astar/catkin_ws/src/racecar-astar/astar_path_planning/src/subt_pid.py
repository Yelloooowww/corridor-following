...
class Robot_PID():
	def __init__(self):
		...
		self.pub_estop = rospy.Publisher("stop_mode", Bool, queue_size = 1)
	...
	def goal_cb(self, p):
		...
		if not self.use_odom:
			...
			if self.goal is not None:
				self.publish_goal(self.goal)
				print('self.publish_goal(self.goal)')
	...
	def publish_goal(self, goal):
		...
		else: marker.header.frame_id = "camera_link"
	...
	def timer_cb(self, event):
		...
		if not self.emergency_stop:
			print('if not self.emergency_stop:')
			self.pub_cmd.publish(self.cmd)
		#self.pub_cmd.publish(self.cmd)
		# print('timer_cb')
