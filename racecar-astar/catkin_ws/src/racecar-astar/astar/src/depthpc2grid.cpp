...
Pc2Grid::Pc2Grid()
{
	...
	// map info parameter
	ros::param::param<std::string>("~map_frame", map_frame, "camera_link");
...
void Pc2Grid::pcCallback(const sensor_msgs::PointCloud2ConstPtr &pc)
{
	...
	try
	{
		...
		listener.waitForTransform(map_frame, pc->header.frame_id, ros::Time(0), timeout);
		listener.lookupTransform(map_frame, pc->header.frame_id, ros::Time(0), transform);
	}
