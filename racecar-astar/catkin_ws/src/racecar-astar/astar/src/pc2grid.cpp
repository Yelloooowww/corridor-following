...
Pc2Grid::Pc2Grid()
{
	...
	sub_map = nh.subscribe("/camera/depth/color/points", 1, &Pc2Grid::pcCallback, this);
}
...
void Pc2Grid::pcCallback(const sensor_msgs::PointCloud2ConstPtr &pc)
{
	...
	try
	{
		...
		listener.waitForTransform("camera_link", pc->header.frame_id, ros::Time(0), timeout);
		listener.lookupTransform("camera_link", pc->header.frame_id, ros::Time(0), transform);
	}
	...
	map.header.stamp = pc->header.stamp;
	map.info.map_load_time = pc->header.stamp;
	map.info.resolution = 0.15;
	map.info.origin.position.x = -3.75;
	map.info.origin.position.y = -3.75;
	map.info.origin.position.z = 0;
